...
const Promotions = () => {
    ...
    useEffect(() => {
        const fetchPromotions = async () => {
            try {
                const response = await axios.get(`${backendUrl}/api/products/promotions`);
                setPromotions(response.data);
                setError('');
            } catch (error) {
                console.error('Error al cargar promociones:', error);
                setError('No se pudieron cargar las promociones. Intenta nuevamente más tarde.');
            }
        };
        fetchPromotions();
    }, [backendUrl]);
    ...
    return (
        <div className="container my-4">
            ...
            {promotions.perros.length > 0 ? (
                ...
                !error && <p className="text-center">No hay productos de perros en promoción.</p>
            )}
            ...
            {promotions.gatos.length > 0 ? (
                ...
                !error && <p className="text-center">No hay productos de gatos en promoción.</p>
            )}
        </div>
    );
};
